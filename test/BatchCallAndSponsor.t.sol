...
contract BatchCallAndSponsorTest is Test {
    ...
    function testDirectExecution() public {
        ...
        // ETH transfer
        calls[0] = HelperApproveAndSwap.Call({to: BOB_ADDRESS, value: 1 ether, data: ""});
        ...
        HelperApproveAndSwap(ALICE_ADDRESS).execute(calls);
        vm.stopPrank();
    ...
    function testSponsoredExecution() public {
        ...
        // Debug nonce
        // console2.log("Nonce before sending transaction:", HelperApproveAndSwap(ALICE_ADDRESS).nonce());
        ...
        bytes32 digest = keccak256(abi.encodePacked(HelperApproveAndSwap(ALICE_ADDRESS).nonce(), encodedCalls));
        ...
        emit HelperApproveAndSwap.CallExecuted(BOB_ADDRESS, calls[0].to, calls[0].value, calls[0].data);

        // As Bob, execute the transaction via Alice's temporarily assigned contract.
        HelperApproveAndSwap(ALICE_ADDRESS).execute(calls, signature);

        // console2.log("Nonce after sending transaction:", HelperApproveAndSwap(ALICE_ADDRESS).nonce());
    ...
    function testReplayAttack() public {
        ...
        uint256 nonceBefore = HelperApproveAndSwap(ALICE_ADDRESS).nonce();
